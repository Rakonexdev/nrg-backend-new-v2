...
class SponsorshipChangeController extends Controller
{
    public function index(Request $request)
    {
        ...
        $dateColumn = ($request->has('tab') && $request->tab === 'approved') ? 'approval_date' : 'created_at';
    ...
    public function destroy($sponsorshipChange)
    {
        try {
            ...
            $id = $sponsorshipChange instanceof SponsorshipChange ? $sponsorshipChange->id : $sponsorshipChange;
            $entity = SponsorshipChange::find($id);

            if ($entity) {
                try {
                    if (!empty($entity->approval_file) && \Illuminate\Support\Facades\Storage::disk('public')->exists($entity->approval_file)) {
                        \Illuminate\Support\Facades\Storage::disk('public')->delete($entity->approval_file);
                    }
                } catch (\Throwable $ignored) {}
                ...
                try {
                    ...
                    \Illuminate\Support\Facades\DB::table('sponsorship_changes')->where('id', $id)->delete();
                }
            } else {
                try {
                    \Illuminate\Support\Facades\DB::table('sponsorship_changes')->where('id', $id)->delete();
            ...
            return response()->json(['message' => 'Sponsorship change deleted successfully']);
        } catch (\Throwable $e) {
            try {
                $id = $sponsorshipChange instanceof SponsorshipChange ? $sponsorshipChange->id : $sponsorshipChange;
                \Illuminate\Support\Facades\DB::table('sponsorship_changes')->where('id', $id)->delete();
            ...
            return response()->json(['message' => 'Error deleting sponsorship change: ' . $e->getMessage()], 500);
        }
    }
