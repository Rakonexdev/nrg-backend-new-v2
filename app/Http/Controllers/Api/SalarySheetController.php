...
class SalarySheetController extends Controller implements HasMiddleware
{
    ...
    {
        return [
            new Middleware('can:view_salary_sheet', only: ['index', 'show']),
            new Middleware('can:salary_sheet_create', only: ['store']),
            new Middleware('can:salary_sheet_edit', only: ['update']),
            new Middleware('can:salary_sheet_delete', only: ['destroy']),
            new Middleware('can:salary_sheet_download', only: ['download']),
        ];
    }
    ...
    public function index(Request $request)
    {
        try {
            ...
            Log::error('Error fetching Salary Sheet records: ' . $e->getMessage());
    ...
    public function store(Request $request)
    {
        try {
            ...
            $path = $file->store('salary_sheets', 'public');

            $record = SalarySheet::create([
            ...
            Log::error('Error creating Salary Sheet: ' . $e->getMessage());
    ...
    {
        $record = SalarySheet::with(['uploader:id,name', 'company:id,name,computer_card'])->findOrFail($id);
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $record = SalarySheet::findOrFail($id);
            ...
            Log::error('Error updating Salary Sheet: ' . $e->getMessage());
    ...
    public function destroy($id)
    {
        try {
            ...
            $record = $id instanceof SalarySheet ? $id : SalarySheet::find($id);
            ...
            if ($record) {
                ...
                try {
                    ...
                    DB::table('salary_sheets')->where('id', $record->id)->delete();
                }
            } else {
                try {
                    DB::table('salary_sheets')->where('id', $id)->delete();
                } catch (\Throwable $ignored) {}
            }
            ...
            Log::error('Failed to delete Salary Sheet', ['error' => $e->getMessage()]);
